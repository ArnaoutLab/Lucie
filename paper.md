...
**Step 1**
The import algorithm first queries the UCIML API to determine if a given dataset can already be imported using the built-in \code{fetch\_ucirepo} function. If so, it returns a tuple containing the string `"uci"` and the result. If not, we use BeautifulSoup~\cite{beautifulsoup} to scrape the UCIMLR for the download link for the corresponding ".zip" file. Then, we extract this archive. 
